...
class TestTopicModeling:
    ...
    def test_run_topic_modeling_basic(self, sample_clustered_adata, annotation_col):
    ...
    def test_run_topic_modeling_filter_unknown(self, sample_clustered_adata, annotation_col):
        ...
        adata.obs.loc[adata.obs.index[:5], annotation_col] = "nan"

        # Test with filtering enabled (default)
        tm.tl.run_topic_modeling(adata, annotation_col=annotation_col, n_topics=3, n_iter=10, filter_unknown=True)
        region_topic1 = adata.uns["topic_modeling"]["region_topic_matrix"]

        # Test with filtering disabled
        tm.tl.run_topic_modeling(adata, annotation_col=annotation_col, n_topics=3, n_iter=10, filter_unknown=False)
    ...
    def test_run_topic_modeling_parameters(self, sample_clustered_adata, annotation_col):
        """Test different parameter combinations."""
        adata = sample_clustered_adata.copy()

        # Test different n_topics
        tm.tl.run_topic_modeling(adata, annotation_col=annotation_col, n_topics=10, n_iter=5, alpha=25, eta=0.05, random_state=42)
    ...
    def test_topic_cluster_matrix_storage(self, sample_clustered_adata, annotation_col):
        """Test that topic-cluster matrix is correctly stored."""
        adata = sample_clustered_adata.copy()

        # Run topic modeling
        tm.tl.run_topic_modeling(adata, annotation_col=annotation_col, n_topics=5, n_iter=10, filter_unknown=False)
    ...
    def test_topic_dbd_matrix_creation(self, sample_clustered_adata, annotation_col):
        ...
        for cluster in count_table.columns:
            ...
            if cluster_mask.any():
                dbd = adata.obs.loc[cluster_mask, annotation_col].iloc[0]
    ...
    def test_topic_modeling_reproducibility(self, sample_clustered_adata, annotation_col):
        """Test that topic modeling results are reproducible with same random state."""
        adata = sample_clustered_adata.copy()

        # Run topic modeling twice with same random state
        tm.tl.run_topic_modeling(adata, annotation_col=annotation_col, n_topics=5, n_iter=10, random_state=42)
        region_topic1 = adata.uns["topic_modeling"]["region_topic_matrix"]
    ...
    def test_topic_modeling_edge_cases(self, sample_clustered_adata, annotation_col):
        """Test edge cases and error conditions."""
        adata = sample_clustered_adata.copy()

        # Test with n_topics = 1
        tm.tl.run_topic_modeling(adata, annotation_col=annotation_col, n_topics=1, n_iter=5)
        topic_results = adata.uns["topic_modeling"]
        model = topic_results["model"]
        region_topic = topic_results["region_topic_matrix"]
        count_table = topic_results["count_matrix"]
    ...
    def test_topic_modeling_with_small_data(self, sample_clustered_adata, annotation_col):
        ...
        tm.tl.run_topic_modeling(adata, annotation_col=annotation_col, n_topics=2, n_iter=5)
        topic_results = adata.uns["topic_modeling"]
        model = topic_results["model"]
        region_topic = topic_results["region_topic_matrix"]
        count_table = topic_results["count_matrix"]
