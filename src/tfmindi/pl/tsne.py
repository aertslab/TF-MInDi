...
def region_tsne(
    ...
    if not palette:
        palette = dict(
            zip(set(region_adata.obs[color_by]), dp.get_colors(len(set(region_adata.obs[color_by]))), strict=False)
        )

    # Plot scatterplot
    plt.figure(figsize=(fig_edge, fig_edge))
    g = sns.scatterplot(
        x=df["x"], y=df["y"], s=dot_size, hue=df["cluster"], palette=palette, ec=None, legend=plot_legend
    )
    ...
    if plot_legend:
        ...
        g.legend(
            [label_handle[label] for label in [label for label in list(palette.keys()) if label in labels]],
